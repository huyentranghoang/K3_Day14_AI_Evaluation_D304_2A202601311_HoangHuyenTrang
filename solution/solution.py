...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        return max(0.0, min(1.0, ap_sum / n_relevant))
...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            values = list((item.get("scores") or {}).values()) if isinstance(item, dict) else []
            numeric = []
            for value in values:
                try:
            ...
            if numeric:
                ...
                response_avgs.append(sum(numeric) / len(numeric))
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        def _avg(attr: str) -> float:
            if not results:
                return 0.0
            return sum(getattr(result, attr) for result in results) / total
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            ...
            if index < len(suggestions):
                ...
                suggested_fix = "Review the full evaluation pipeline"
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Clarify the system prompt so answers stay on the user question"
            )
        ...
        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Improve intent detection so the assistant stays on the requested topic"
            )

        defaults = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Improve retrieval ranking so relevant evidence appears before noise",
            "Add a faithfulness guardrail that refuses claims not present in retrieved context",
        ]
